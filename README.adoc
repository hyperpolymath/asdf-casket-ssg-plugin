...
image:https://img.shields.io/github/v/release/hyperpolymath/asdf-casket-ssg-plugin?style=flat-square[Release,link={url-repo}/releases]
...
[.lead]
An {url-asdf}[asdf] plugin to install and manage {url-tool}[Casket Ssg] versions.
...
Casket Ssg is an upstream tool managed by this plugin. Refer to the official
...
|**Version Switching**
|Switch between Casket Ssg versions per project

|**Project Isolation**
|Pin Casket Ssg per repository via `.tool-versions`

|**Reproducible Builds**
|Align CI/CD and developer environments to the same Casket Ssg version
...
=== Installing Casket Ssg Versions

[source,bash]
----
asdf list all casket-ssg
asdf install casket-ssg latest
asdf install casket-ssg 0.1.0
----
...
[source,bash]
----
asdf global casket-ssg 0.1.0
asdf local casket-ssg 0.1.0
asdf current casket-ssg
casket-ssg --version
----
...
* Ensure `casket-ssg` is on PATH after installation.
* Verify the installed version with `casket-ssg --version`.
